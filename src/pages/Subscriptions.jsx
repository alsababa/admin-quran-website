        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-400 text-sm">Manage premium access and billing.</p>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <StatCard icon={<TrendingUp className="text-green-500" />} label="Monthly Revenue (Est.)" value={`${stats.totalRevenue} SAR`} />
                <StatCard icon={<CheckCircle className="text-blue-500" />} label="Active Plans" value={stats.active} />
                <StatCard icon={<AlertCircle className="text-amber-500" />} label="Expired/Pending" value={stats.pending} />
            </div>

            <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden mt-8">
                <div className="px-6 py-4 border-b border-gray-800 flex justify-between items-center">
                    <h4 className="font-semibold">Premium Users</h4>
                ...
                <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-gray-800">
                        {loading ? (
                            <tr><td colSpan="4" className="px-6 py-10 text-center text-gray-500">Loading subscriptions...</td></tr>
                        ) : subscriptions.length === 0 ? (
                            <tr><td colSpan="4" className="px-6 py-10 text-center text-gray-500">No premium users found.</td></tr>
                        ) : (
                                ...
                                <tr key={user.id} className="hover:bg-gray-800 hover:bg-opacity-30 transition-colors">
                                    <td className="px-6 py-4">
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500">{user.subscriptionProductId || 'N/A'}</p>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className="text-xs">{user.subscriptionPlatform || 'Google Play'}</span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className="text-xs font-mono text-gray-400">
                                            {user.subscriptionEndDate ? new Date(user.subscriptionEndDate).toLocaleDateString() : 'Permanent'}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
