...
export default function PaymentsPage() {
    ...
    if (loadingPayments || loadingUsers) {
        return (
            <div className="flex flex-col items-center justify-center py-20">
                ...
                <p className="text-[#5AA564]/70 font-semibold">جاري جلب بيانات المدفوعات والمشتركين...</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            {/* Header Section */}
            <div className="glass-panel p-6 rounded-3xl border border-[#5AA564]/10 flex flex-col md:flex-row md:items-center justify-between gap-4 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-[#5AA564]/10 blur-3xl rounded-full" />
                
                <div className="flex items-center gap-4 relative z-10">
                    <div className="w-14 h-14 bg-gradient-to-br from-[#5AA564]/20 to-[#4A8F53]/20 rounded-2xl flex items-center justify-center border border-[#5AA564]/20 shadow-[0_0_20px_rgba(90,165,100,0.15)]">
                        <CreditCard size={28} className="text-[#5AA564]" />
                    </div>
                    <div>
                        <h1 className="text-2xl font-black text-white tracking-tight">سجل المدفوعات والاشتراكات</h1>
                        <p className="text-sm font-medium text-[#5AA564]/60 mt-1">متابعة مدفوعات ميسر وإدارة اشتراكات المستخدمين.</p>
                    </div>
                </div>

                <div className="relative z-10 w-full md:w-auto">
                    <div className="flex items-center bg-[#1E2448]/80 border border-[#5AA564]/15 rounded-xl px-4 py-2 focus-within:ring-2 focus-within:ring-[#5AA564]/30 transition-all">
                        <Search size={18} className="text-[#5AA564]/50" />
                        <input
                            type="text"
            ...
            <div className="bg-[#5AA564]/10 border border-[#5AA564]/20 rounded-2xl p-4 flex items-start gap-4">
                <ExternalLink className="text-[#5AA564] shrink-0 mt-0.5" size={20} />
                <div>
                    <h3 className="text-white font-bold text-sm">لوحة تحكم ميسر الرسمية</h3>
                    <p className="text-[#5AA564]/70 text-xs mt-1 leading-relaxed">
            ...
            {/* Table Section */}
            <div className="glass-panel border border-[#5AA564]/10 rounded-3xl overflow-hidden shadow-2xl relative">
                <div className="overflow-x-auto">
                    <table className="w-full text-right border-collapse">
                        <thead>
                            <tr className="bg-[#1A2035]/80 border-b border-[#5AA564]/10">
                                <th className="px-6 py-4 text-xs font-black text-[#5AA564]/70 uppercase tracking-widest whitespace-nowrap">رقم الدفع المرجعي</th>
                                <th className="px-6 py-4 text-xs font-black text-[#5AA564]/70 uppercase tracking-widest">المشترك / الباقة</th>
                                <th className="px-6 py-4 text-xs font-black text-[#5AA564]/70 uppercase tracking-widest">التواريخ</th>
                                <th className="px-6 py-4 text-xs font-black text-[#5AA564]/70 uppercase tracking-widest">الحالة</th>
                                <th className="px-6 py-4 text-xs font-black text-[#5AA564]/70 uppercase tracking-widest">إجراءات الإدارة</th>
                            </tr>
                        </thead>
                                ...
                                {enrichedPayments.length > 0 ? enrichedPayments.map((payment, index) => {
                                    ...
                                    return (
                                        <motion.tr
                                            ...
                                            className="hover:bg-[#5AA564]/5 transition-colors group"
                                        >
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col">
                                                    ...
                                                    <span className="text-xs text-[#5AA564]/50">{payment.user?.email}</span>
                                                    <div className="mt-2 inline-flex items-center px-2 py-0.5 rounded-md bg-[#1E2448] border border-[#5AA564]/20 text-[10px] text-[#5AA564] max-w-max">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col gap-1">
                                                    <div className="flex items-center gap-1.5 text-xs text-white/70">
                                                        <Clock size={12} className="text-[#5AA564]/50" />
                                                        <span>تاريخ الدفع: {payDate}</span>
                                                    </div>
                                                    <div className="flex items-center gap-1.5 text-xs text-[#5AA564]">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${isPaid ? 'bg-[#5AA564]/10 text-[#5AA564] border-[#5AA564]/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'}`}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <div className="flex items-center gap-2 opacity-100 lg:opacity-60 lg:group-hover:opacity-100 transition-opacity">
                                                    <button 
                                                        ...
                                                        className="px-3 py-1.5 bg-[#5AA564]/10 hover:bg-[#5AA564]/20 text-[#5AA564] rounded-lg text-xs font-bold transition-colors flex items-center gap-1.5 border border-[#5AA564]/20 disabled:opacity-50"
                                                    >
                                    ...
                                    <tr>
                                        <td colSpan="5" className="px-6 py-20 text-center text-[#5AA564]/40 font-bold border-b border-[#5AA564]/5 flex flex-col items-center gap-3">
